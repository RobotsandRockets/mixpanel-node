...
var create_client = function (token, config) {
    ...
    /**
     send_raw_export_request(data, callback)
     ---
     this function sends an async GET request to mixpanel export without parsing the response

     data:object                     the data to send in the request
     callback:function(err:Error)    callback is called when the request is finished or an error occurs
     */
    metrics.send_raw_export_request = function (data, callback) {
        callback = callback || function() {};
        ...
        request_options.path = ["/api/2.0/export/?", query].join("");
        ...
        http.get(request_options,function (res) {
            ...
            res.on('end', function () {
                ...
                if(response.error !== undefined) {
                    var err = new Error(response.error + " for request: " + response.request);
                    if (metrics.config.debug) {
